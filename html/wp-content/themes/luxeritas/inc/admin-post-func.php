...
add_action( 'after_setup_theme', function() {
	...
	foreach( $color_palette as $key => $val ) {
		if( isset( $luxe['block_palette_color_' . $i] ) ) {
			$color_palette[$key]['color'] = $luxe['block_palette_color_' . $i];
		}
		if( isset( $luxe['block_palette_name_' . $i] ) ) {
			$color_palette[$key]['name'] = $luxe['block_palette_name_' . $i];
		}
		++$i;
	}
	...
	for( $i = $c; $c + 6 > $i; ++$i ) {
		...
		if( isset( $luxe['block_palette_color_' . $i] ) ) {
			$ca['color'] = $luxe['block_palette_color_' . $i];
			if( isset( $luxe['block_palette_name_' . $i] ) ) {
				$ca['name'] = $luxe['block_palette_name_' . $i];
			}
		}
		if( !empty( $ca ) ) {
			$color_palette[$i] = $ca;
