...
<ul class="subsubsub">
		...
		<form id="ahm-options-form" method="post">
			...
			<table class="form-table">
				<tr>
					...
					<td>
						<?php $this->print_radio_options( $options_content_end_marker, 'content_end_marker', $is_premium, $is_premium ? null : 0 ); ?>
					</td>
				</tr>
				<tr>
					<th scope="row">
	...
	public function ajax_aurora_heatmap() {
		...
		foreach ( $dataset as $data ) {
			...
			$add  = array(
				...
				'x'         => $to_int( $data['x'] ),
				'y'         => $to_int( $data['y'] ),
				'width'     => $to_int( $data['width'] ),
				'height'    => $to_int( $data['height'] ),
				'insert_at' => ( new DateTime( sprintf( '%d second', $to_int( $data['time'] ) ), $tz ) )->format( 'Y-m-d H:i:s' ),
			);

			// Skip if low accuracy.
			if ( $this->get_accuracy( $add['width'], $event_id ) < $this->options['accuracy'] ) {
				continue;
			}
	 ...
	 * Get access from
	 *
	 * @return string pc|tablet|mobile
	 */
	protected function get_access_from() {
		static $access;
	 ...
	 */
	protected function enqueue_report() {
		if ( ! $this->can_report() ) {
			return;
		}
		...
		wp_enqueue_script(
			...
			plugins_url( 'js/reporter.js', __FILE__ ),
			array( 'jquery' ),
			$this::VERSION,
			false
		);
	 ...
	 */
	protected function enqueue_view() {
		if ( ! $this->can_view() ) {
			return;
		}
		...
		wp_enqueue_script(
			...
			plugins_url( 'js/h337.js', __FILE__ ),
			array( 'jquery' ),
			$this::VERSION,
			false
		);
		...
		wp_enqueue_script(
			'aurora-heatmap-chroma',
			plugins_url( 'js/chroma.min.js', __FILE__ ),
			array(),
			$this::VERSION,
			false
		);
		...
		wp_enqueue_script(
			...
			plugins_url( 'js/viewer.js', __FILE__ ),
			array( 'jquery' ),
			$this::VERSION,
			false
		);
