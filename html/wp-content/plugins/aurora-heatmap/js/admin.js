	...
	var self = {
		view: function() {
			...
			var events = [
				{
					column: 'click',
					legend: args.click_heatmap,
				},
				{
					column: 'breakaway',
					legend: args.breakaway_heatmap,
				},
				{
					column: 'attention',
					legend: args.attention_heatmap,
				},
			];
			...
			var w;
			Array.prototype.forEach.call( document.getElementsByClassName( 'ahm-view' ), function( e ) {
		...
		settings: function() {
			function radio_group_disable( e ) {
				return function( ev ) {
					e.form[e.name].forEach( function( r ) {
						var n = r.nextElementSibling.children[1];
						var is_disabled = n.classList.contains( 'disabled' ) || r.disabled || ! r.checked;

						Array.prototype.forEach.call( n.querySelectorAll( '.inner-label' ), function( i ) {
							i.style.opacity = is_disabled ? '.6' : '1';
						} );

						Array.prototype.forEach.call( n.querySelectorAll( 'input[type="text"]' ), function( i ) {
			...
			Array.prototype.forEach.call( rg, function( e ) {
			...
			document.getElementById( 'ahm-options-form' ).addEventListener( 'keydown', function( event ) {
				if ( 13 === event.which ) {
